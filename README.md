...
## Cert verification & price guide (graded cards, PSA / CGC / PriceCharting / Collectr — via Parse.bot)

For graded slabs with a cert number filled in, the app checks the grading company's *own* site directly — the real source of truth for that exact card, not a generic database entry. It also pulls a second, independent price signal from PriceCharting and Collectr, alongside the eBay/SoldComps average. All four go through **Parse.bot** — one account, one API key, no per-site setup.

**Why Parse.bot instead of hitting each site directly:** PSA, CGC, PriceCharting, and Collectr don't all offer their own clean developer APIs (PSA's official one exists but is built around a login-derived token that silently expires; CGC has none at all). Parse.bot maintains managed, monitored REST wrappers over each site instead — they handle the anti-bot fight, proxy rotation, and site-maintenance burden on their end, and automatically repair an endpoint if the underlying site's layout changes. That's meaningfully more reliable than either scraping these sites ourselves or depending on PSA's expiring token.

**Setup:** sign up free at [parse.bot](https://parse.bot) (no card required), grab your API key (starts with `pmx_`), add it to Netlify as `PARSE_API_KEY`, redeploy. **Free tier: 200 credits/month, 5 req/min.**

**What this actually costs per item** — PSA/CGC cert lookup is 1 credit; PriceCharting is 1 credit; Collectr is 3 credits (a search call + a details call). Cert verification runs automatically for every graded item (cheap, and clearly wanted if it's graded). The **price guide (PriceCharting + Collectr, ~4 credits)** is opt-in per item instead — there's a "Check premium price guide" toggle on the Add/Edit form (off by default), plus a link to turn it on later from an item's detail page. Turn it on for the high-end pieces where a second price opinion is worth the credits; leave it off for everyday cards. A graded item with the toggle on costs ~5 credits total; a raw card with it on costs ~4; either one with it off costs just the cert check (or nothing, for raw).

For reference: at that rate, ~40 credits-worth toggled on gets you roughly 8–10 items on the free tier per month. If you're populating an existing collection of, say, 100+ items with the toggle on for most of them, the $30/mo Hobby tier (1,000 credits) covers that comfortably in one sitting — its 20 req/min rate limit also matters more than the monthly total if you're rapid-adding many items in one session, since the free tier's 5 req/min will otherwise throttle you well before you hit the monthly cap.

- **PSA** — cert lookup returns the confirmed grade, full card description, population count, and **an actual photo PSA took of that specific card**, not a generic stock image.
- **CGC** — cert lookup returns the confirmed grade, card identity, and Obverse/Reverse reference images straight from CGC's own image hosting. This is the one that used to be essentially unusable (their site blocks plain scraping); Parse.bot fixes that entirely.
- **PriceCharting** — current prices at ungraded / Grade 9 / PSA 10 for the matched card, shown alongside the eBay-based asking price as a second opinion.
- **Collectr** — current market price and per-grading-company graded prices (PSA/BGS/CGC), plus recent price-history snapshots.
- **BGS and SGC** aren't wired up yet — cert lookups for those show a clear "not supported yet" note instead of failing quietly. (Parse.bot does have an SGC wrapper if you ever want to add it later — same account, same key.)

Without `PARSE_API_KEY` configured, cert verification and the price guide simply don't run — everything else in the app (eBay/SoldComps pricing, the Pokémon TCG card database, Auto-Identify) works exactly the same either way.

A cert check and a price-guide check each only ever run automatically **once** per item — after that first attempt (success or failure), neither silently retries itself just from viewing or re-saving the item. Tap the refresh icon on the item's detail page to try again on purpose.
