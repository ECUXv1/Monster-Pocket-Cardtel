...
- **PSA** works two ways. Best is PSA's real, official Public API — register free at [psacard.com/publicapi](https://www.psacard.com/publicapi) to get a bearer token, add it to Netlify as `PSA_API_TOKEN`, and redeploy. This gets you a proper structured JSON response (no bot-blocking risk), the confirmed grade and full card description, population counts, and — genuinely the best part — **an actual photo PSA took of that specific card**, not a generic stock image. Without a token, it falls back to reading PSA's public cert page directly, which usually works but can occasionally get blocked by their bot detection (a 403) since it's not an official channel; that fallback also picks up a bonus most people don't get anywhere else — recent eBay sold prices for that exact card + grade population, shown right on PSA's own cert page.

  **Important limit if you use the real API:** PSA's token comes with a hard **100 requests per day**, shared across everything using that token — including testing calls you make yourself on PSA's Swagger docs page. Each cert check uses up to 2 of those (card data + the photo lookup). To respect that: a cert check only ever runs automatically **once** per item — after that first attempt (success or failure), it won't silently retry itself just from viewing or re-saving the item. You have to tap the refresh icon on the item's detail page to try again on purpose. If you do exhaust the quota, PSA's response tells you exactly when it resets (usually within a matter of hours, not a full day from whenever you happened to hit it).

  **More than one token?** Add `PSA_API_TOKEN_2`, `PSA_API_TOKEN_3`, etc. (any number) to give the app more than one token to try — e.g. if a family member or friend with their own separate PSA account is comfortable sharing their token's spare quota with you. If one token hits its daily limit or fails, the next one in the list gets tried automatically before falling back to the free scraper. Worth knowing: PSA's own terms state each *person* may only have a single Collectors Account — this only helps if each token genuinely belongs to a different real account holder who's actually agreed to it, not several accounts created by one person to get around the limit.
- **CGC** is best-effort either way — no equivalent public API was found. Their cert page (`cgccards.com/certlookup/{cert}/`) runs bot detection that can block even a plain request, and part of the page loads via client-side JavaScript that a simple fetch never sees. When it works, you get the confirmed grade and description; when it doesn't, the item's detail page says so plainly (with a link to check `cgccards.com/certlookup` yourself) rather than showing broken or missing data silently.
